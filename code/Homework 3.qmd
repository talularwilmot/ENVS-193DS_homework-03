...
To evaluate the strength of the relationship between pickleweed biomass and soil salinity, I used a Pearson's correlation because because both variables (soil salinity in mS/cm and pickleweed biomass in grams) are continuous, normally distributed, and showed linearity with each other. The test showed that there was significantly correlation between salinity (mS/cm) and pickleweed biomass (Pearson's correlation, t(21) = 2.9, p = 0.009, ⍺ = 0.05, 95% CI: 0.16, 0.78). 
...
The Spearman's correlation also shows significant correlation between salinity (mS/cm) and pickleweed biomass (g) (rho = 0.59, S = 824, p = 0.003, ⍺ = 0.05). These results led us to the same conclusion of rejecting the null hypothesis that there is no relationship between the variables, however this test uses rank based values rather than raw data so it only asses monotonic relationships not necessarily showing us that the variables are changing at a constant rate. 
