...
The two appropriate tests to determine the strength of the relationship between salinity and California pickleweed biomass are **Pearson’s correlation** and **Spearman's correlation**. Pearson’s correlation measures the strength and direction of the linear relationship between two continuous variables salinity and California Pickleweed biomass. The Spearman's correlation measures the monotonic relationship, (if it is linear or not) based on the ranked values rather than the raw data and does not show the direction of the relationship.
...
ggplot(data = salinity, # starting data frame
       aes(sample = `pickleweed`)) + # show normality of pickleweed variable for QQ plot (no x-axis)
 geom_qq() +
  geom_qq_line()

ggplot(data = salinity, # starting data frame
       aes(sample = `salinity_mS_cm`)) + # show normality of salinity variable for QQ plot (no x-axis)
 geom_qq() + 
  geom_qq_line()

```

I checked the assumptions of normality of the continuous variables using QQplots, and linearity using the scatterplot shown in 1b. The QQplots follow a linear trend showing normality with no significant outliers, and the visualization shows a linear trend for the data points using salinity as the predictor and pickleweed as the response.
...
To evaluate the strength of the relationship between pickleweed biomass and soil salinity, I used a Pearson's correlation because because both variables (soil salinity in mS/cm and pickleweed biomass in grams) are continuous, normally distributed, and showed linearity with each other. We found a moderately strong postitive relationship (Pearson's r = 0.53) between salinity (mS/cm) and pickleweed biomass.

## e. Test implications

Our Pearson's correlation indicates a significant positive linear relationship between salinity (mS/cm) and pickleweed biomass (g), meaning that in order for pickleweed plants to be successful they need to be in saltier conditions. This means that in the event of extreme rain or freshwater input plants may struggle to survive and potentially artificial salt addition may be necessary. This study would benefit from further sampling to solidify this relationship, but without a visible plateau in our scatterplot visualization out planters should lean towards higher salinity.

## f. Double check your own work

```{r}
cor.test(salinity$salinity_mS_cm, salinity$pickleweed, #use salinity data to test for correlation
         method = "spearman") #use spearman correlation test 
```

The Spearman's correlation also shows a moderately strong relationship (rho = 0.59) between salinity (mS/cm) and pickleweed biomass. These results led us to the same conclusion as the Pearson's correlation of rejecting the null hypothesis that there is no relationship between the variables, however Pearson's correlation showed a slightly weaker relationship betweent he variables (r = 0.53). The Spearman's test uses rank based values rather than raw data so it only asses monotonic relationships not necessarily showing us that the variables are changing at a constant rate.
...
![Table](Stat_critique_table%202.png)
